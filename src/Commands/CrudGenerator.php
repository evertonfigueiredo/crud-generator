...
class CrudGenerator extends GeneratorCommand
{
    ...
    protected $signature = 'make:crud
                            {name : Nome da tabela}
                            {stack : o layout de desenvolvimento que deve ser instalada (bootstrap,tailwind,livewire,api)}
     ...
     * @var string
     */
    protected $description = 'Criar operações Laravel CRUD';
    ...
    {
        $this->info('Executando o Gerador Crud ...');

        $this->table = $this->getNameInput();

        // If table not exist in DB return
        if (! $this->tableExists()) {
            $this->error("`$this->table` tabela não existe.");
    ...
    protected function promptForMissingArgumentsUsing(): array
    {
        return [
            'stack' => fn() => select(
                label: 'Qual o layout de desenvolvimento você gostaria de instalar?',
                options: [
                    'bootstrap' => 'Blade com Bootstrap css',
                    'tailwind' => 'Blade com Tailwind css',
                    'livewire' => 'Livewire com Tailwind css',
                    'api' => 'API',
                ],
                scroll: 4,
            ),
    ...
    protected function writeRoute(): static
    {
        ...
        $this->info('Adicione a rota abaixo: i:e; web.php ou api.php');
    ...
    {
        $this->info('Criando componente Livewire ...');
    ...
    protected function buildModel(): static
    {
        ...
        if ($this->files->exists($modelPath) && $this->ask('Já existe Model. Você quer substituir (y/n)?', 'y') == 'n') {
            return $this;
        }

        $this->info('Criando Models ...');
        ...
        $this->info('Criando classe de solicitações ...');
